...
@torch.jit.script
def axisangle2quat(vec, eps=1e-6):   #将轴角（axis-angle）旋转表示转换为四元数（quaternion）表示
    """
...
class FrankaCubeStack(VecTask):
    ...
    def create_sim(self):
        ...
        self.sim = super().create_sim(
            self.device_id, self.graphics_device_id, self.physics_engine, self.sim_params)  #self.gym.create_sim
...
def compute_franka_reward(
    ...
    # distance from hand to the cubeA   距离A的reward
    ...
    # reward for lifting cubeA  移动A的reward
    ...
    # how closely aligned cubeA is to cubeB (only provided if cubeA is lifted) 方块底面中心对齐reward
    ...
    # 成功堆叠奖励 必须三项同时满足 cubeA和B的xy对齐 堆叠高度与目标高度一致 手爪松开
